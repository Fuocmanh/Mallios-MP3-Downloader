    ...
    try:
        ...
        with PROGRESS_LOCK:
            if returncode in {0, 101}:
                ...
                # Rút gọn lỗi để FE không hiển thị nguyên traceback yt-dlp.
                output_lower = (combined_output or "").lower()
                if "rate-limited" in output_lower or "rate limited" in output_lower:
                    clean_err = "YouTube đang giới hạn truy cập; bài này được đánh dấu lỗi và tiến trình vẫn tiếp tục."
                elif "video unavailable" in output_lower or "content isn.t available" in output_lower:
                    clean_err = "Video không khả dụng trên YouTube; bài này được đánh dấu lỗi và tiến trình vẫn tiếp tục."
                else:
                    err_lines = [l.strip() for l in (combined_output or "").splitlines() if "ERROR:" in l or "Warning:" in l]
                    clean_err = err_lines[-1].replace("ERROR:", "").strip() if err_lines else "Không thể tải bài này."
