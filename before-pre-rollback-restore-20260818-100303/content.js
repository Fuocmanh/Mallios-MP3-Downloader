...
function getFileListOutput() {
  try {
    ...
    function preloadPlaylistStreams(items) {
      ...
      // 1. Chỉ gửi danh sách để backend preload tối thiểu, tránh rate-limit YouTube
      ...
      // 2. Chỉ prefetch bài đầu tiên; các bài khác chỉ lấy preview khi người dùng bấm phát
      const topUrls = urls.slice(0, 1);
      topUrls.forEach((u, i) => {
        setTimeout(() => {
