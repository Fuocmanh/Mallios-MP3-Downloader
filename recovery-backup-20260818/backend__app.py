...
def get_playlist():
    ...
    try:
        ...
        is_playlist = bool(query.get("list")) or "/playlist" in parsed_url.path
        args = ["--dump-single-json", "--extractor-args", "youtube:player_client=android,web"]
        if is_playlist:
            # Lay metadata toan bo playlist; khong tai audio nen khong anh huong preview.
            args.extend(["--flat-playlist", "--yes-playlist", "--ignore-errors", "--no-warnings"])
        else:
                ...
                for original_file in new_files:
                    if original_file.is_file():
                        ...
                        if save_target == "drive":
                            ...
                            # Bao cho giao dien biet ngay khi Drive da xac nhan file.
                            # Worker van tiep tuc ghi history va don dep file tam.
                            with PROGRESS_LOCK:
                                PARALLEL_PROGRESS[state_key]["percent"] = 100.0
                                PARALLEL_PROGRESS[state_key]["status"] = "uploaded"
                                PARALLEL_PROGRESS[state_key]["message"] = "Da upload len Google Drive. Dang hoan tat hau xu ly..."
...
def run_parallel_downloads_background(links: list[str], quality: str, save_folder: Path, max_files: int, save_target: str = "local", options: dict = None):
    ...
    max_workers = 3 if save_target == "local" else 2
            ...
            while True:
                ...
                with PROGRESS_LOCK:
                    ...
                    if uploaded_count > 0:
                        PROGRESS_STATE["status"] = "completed"
                        PROGRESS_STATE["percent"] = 100.0
                        PROGRESS_STATE["message"] = f"Da upload {uploaded_count}/{total_files} bai len Google Drive. Dang hoan tat hau xu ly..."
...
def _reconcile_stale_progress_locked():
    """Finalize stale running state only after every recorded worker is terminal."""
    if PROGRESS_STATE.get("status") != "running" or not PARALLEL_PROGRESS:
        return
    terminal = {"completed", "uploaded", "failed", "cancelled"}
    states = list(PARALLEL_PROGRESS.values())
    if any(state.get("status") not in terminal for state in states):
        return
    with ACTIVE_PROCESSES_LOCK:
        if ACTIVE_PROCESSES:
            return
    failed = [state for state in states if state.get("status") == "failed"]
    uploaded = sum(1 for state in states if state.get("status") in {"completed", "uploaded"})
    total = len(states)
    PROGRESS_STATE["status"] = "failed" if failed else "completed"
    PROGRESS_STATE["percent"] = round(uploaded * 100.0 / total, 1) if total else 100.0
    if not failed:
        PROGRESS_STATE["percent"] = 100.0
        if not PROGRESS_STATE.get("message") or "Đang" in PROGRESS_STATE.get("message", ""):
            PROGRESS_STATE["message"] = "Hoàn thành."
    else:
        PROGRESS_STATE["error"] = "; ".join(state.get("message", "") for state in failed if state.get("message"))
        PROGRESS_STATE["message"] = "Tải thất bại."

@app.route("/api/progress", methods=["GET"])
def get_progress():
    with PROGRESS_LOCK:
        _reconcile_stale_progress_locked()
...
class PreviewLogger:
    ...
    def error(self, msg):
        if 'Requested format is not available' not in str(msg):
            print(f'yt-dlp preview: {msg}', file=sys.stderr)
...
def preview_info():
    ...
    if not direct_url:
                return response({"status": "error", "code": "preview_unavailable", "message": "Không thể lấy luồng nghe thử. Video có thể yêu cầu đăng nhập, xác nhận độ tuổi hoặc bị giới hạn quyền truy cập."}, 422)
