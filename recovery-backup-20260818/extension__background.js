...
async function proxyApiRequest(path, options = {}) {
  ...
  try {
    ...
    return {
      ok: false,
      status: 503,
      body: JSON.stringify({ status: "error", message: "M├íy chß╗º Mallios ch╞░a bß║¡t. Vui l├▓ng mß╗ƒ tß╗çp run.bat trong th╞░ mß╗Ñc Mallios." })
    };
  }
}
      ...
      if (chrome.identity && chrome.identity.getAuthToken) {
        chrome.identity.getAuthToken({ interactive: true }, async (token) => {
          if (chrome.runtime.lastError || !token) {
            const err = chrome.runtime.lastError?.message || "Kh├┤ng nhß║¡n ─æ╞░ß╗úc token tß╗½ Chrome";
        ...
        resolve({ ok: false, error: "chrome.identity kh├┤ng khß║ú dß╗Ñng" });
      }
    } catch (e) {
  ...
  try {
    chrome.notifications.clear(notifId, () => {
      chrome.notifications.create(notifId, {
        ...
        message: message || "Tß║úi nhß║íc ho├án tß║Ñt!",
        priority: priority
      });
    });
  } catch (_) {}
...
chrome.runtime.onInstalled.addListener(() => {
  try {
    chrome.contextMenus.create({
      ...
      title: "ΓÜí Tß║úi MP3 bß║▒ng Mallios",
...
chrome.contextMenus.onClicked.addListener(async (info, tab) => {
  if (info.menuItemId === "mallios-download-context") {
    ...
    showSingleNotification("ΓÜí Mallios MP3", "─Éang tß║úi b├ái h├ít trong nß╗ün...", 1);

    // Ph├ít s├│ng sß╗▒ kiß╗çn bß║»t ─æß║ºu tß║úi ngay tß╗¢i c├íc tab content script
    broadcastToTabs({ type: "mallios-download-started", url: targetUrl });

    try {
      // ─Éß╗ìc c├íc thiß║┐t lß║¡p ng╞░ß╗¥i d├╣ng ─æ├ú l╞░u tß╗½ giao diß╗çn tiß╗çn ├¡ch
      ...
      if (data.status === "success") {
        console.log("[Mallios Context Menu] ─É├ú gß╗¡i lß╗çnh tß║úi th├ánh c├┤ng:", targetUrl);

        // Theo d├╡i tiß║┐n tr├¼nh tß║úi ngß║ºm v├á gß╗¡i th├┤ng b├ío khi ho├án tß║Ñt
        let pollCount = 0;
        const interval = setInterval(async () => {
          pollCount++;
          if (pollCount > 1200) { // Tß╗æi ─æa 10 ph├║t cho video/playlist d├ái
          ...
          try {
            ...
            // ─Éß╗ông bß╗Ö tiß║┐n tr├¼nh li├¬n tß╗Ñc tß╗¢i giao diß╗çn n├║t nß╗òi tr├¬n web
            ...
            if (progData.status === "completed") {
              ...
              showSingleNotification("≡ƒÄë Mallios MP3 - Ho├án tß║Ñt!", progData.message || "─É├ú tß║úi b├ái h├ít th├ánh c├┤ng!", 2);
            } else if (progData.status === "failed") {
              clearInterval(interval);
              broadcastToTabs({ type: "mallios-download-failed", data: progData });
              showSingleNotification("Γ¥î Mallios MP3 - Lß╗ùi tß║úi nhß║íc", progData.message || "Tß║úi b├ái h├ít thß║Ñt bß║íi!", 2);
        ...
        showSingleNotification("ΓÜá∩╕Å Mallios MP3", data.message || "M├íy chß╗º ─æang bß║¡n xß╗¡ l├╜ l╞░ß╗út tß║úi kh├íc.", 1);
      }
    } catch (e) {
      console.warn("[Mallios Context Menu] Lß╗ùi gß╗¡i lß╗çnh tß║úi:", e);
    }
  }
});
