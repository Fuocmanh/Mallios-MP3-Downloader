  ...
  try {
    ...
    function applyStorageTarget(target) {
      ...
      if (target === 'drive') {
        ...
        checkDriveStatus();
    // Backend co the mat vai giay de khoi dong; tu dong doc lai cau hinh da luu.
    setTimeout(checkDriveStatus, 1500);
    setTimeout(checkDriveStatus, 4000);
      } else {
    ...
    function normalizeProgressData(data) {
      ...
      const activeMessage = /Đang tải|Đang chuẩn bị|Đang gửi yêu cầu|Đang chạy/.test(message);
      const uploadMatch = message.match(/(?:Đã upload|Da upload|Uploaded)\s+(\d+)\s*\/\s*(\d+)/i);
      const uploadInProgress = uploadMatch && Number(uploadMatch[1]) < Number(uploadMatch[2]);
      if (percent < 100 && uploadInProgress && normalized.status === 'completed') normalized.status = 'running';
      if (percent < 100 && activeMessage && normalized.status === 'completed') normalized.status = 'running';
      return normalized;
    }
    ...
    function startProgressPolling(btn, originalContent) {
      ...
      progressInterval = setInterval(async () => {
        try {
          ...
          const data = await res.json();
          applyProgressData(normalizeProgressData(data));
        } catch (err) {
          if (progressInterval) {
            clearInterval(progressInterval);
            progressInterval = null;
    ...
    async function checkInitialProgress() {
      ...
      try {
        const res = await apiFetch('/api/progress', { method: 'GET' });
        const data = await res.json();
        const activeMessage = String(data.message || '');
        const looksActive = Number(data.percent || 0) < 100 && /Đang tải|Đang chuẩn bị|Đang gửi yêu cầu|Đang chạy/.test(activeMessage);
        if (looksActive && data.status === 'completed') data.status = 'running';
        applyProgressData(data);
        if (data.status === 'running' && !progressInterval) startProgressPolling(null, null);
      } catch (_) {}
    }
    ...
    async function performDownload(payload, btn, originalContent) {
      ...
      // Bắt đầu polling ngay, không chờ POST /download trả về.
      startProgressPolling(btn, originalContent);
      try {
        const requestWatchdog = setTimeout(() => {
          if (!progressInterval) startProgressPolling(btn, originalContent);
        }, 1200);
    ...
    function extractFromYtInitialData() {
      ...
      const isPlaylistPage = /[?&]list=/.test(location.search) || location.pathname === '/playlist';
      ...
      function parseRenderer(r) {
        if (!r) return;
        const v = r.playlistPanelVideoRenderer || r.playlistVideoRenderer || (isPlaylistPage ? null : (r.compactVideoRenderer || r.videoRenderer || r.gridVideoRenderer || r.musicResponsiveListItemRenderer));
        if (!v) return;
        const vId = v.videoId;
        let title = '';
        if (v.title) {
      ...
      function traverseObject(obj, depth = 0) {
        ...
        if (!isPlaylistPage && obj.videoId && (obj.title || obj.headline)) {
